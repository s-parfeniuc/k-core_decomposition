...
struct Node {
    ...
    neighbors: Vec<(usize, Arc<Mutex<MessageMail>>)>, // lista di Message_Mail dei vicini,
...
impl Node {
    pub fn new(n: usize) -> Self {
        Node {
            ...
            est: Estimates::new(),
            neighbors: Vec::new(),
            messages: Arc::new(Mutex::new(MessageMail::new())),
            changed: true,
        }
    }

    pub fn add_neighbor(&mut self, node: Arc<Mutex<MessageMail>>, id: usize) {
        // aggiunge un nuovo vicino
        self.neighbors.push((id, node));
    }
    ...
    pub fn send_messages(&mut self) {
        if self.changed {
            ...
            for neighbor in &mut self.neighbors {
                if self.coreness < self.est.get(neighbor.0) {
                    /*println!(
                        "Sono {}: mando a {}(estimate {}) la mia nuova coreness {}",
                        self.id,
                        neighbor.0,
                        self.est.get(neighbor.0),
                        self.coreness
                    );*/
                    neighbor
                        .1
...
impl Graph {
    ...
    pub fn add_edge(&mut self, i: usize, j: usize) {
        ...
        self.inmap[i].add_neighbor(Arc::clone(&message_j), j);
        self.inmap[j].add_neighbor(Arc::clone(&message_i), i);
    }
